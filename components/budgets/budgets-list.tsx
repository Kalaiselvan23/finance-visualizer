  ...
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {budgets.map((budget:Budget) => {
        const percentage = budget.amount ? Math.round((budget.spent / budget.amount) * 100) : 0
        const isOverBudget = budget.spent > budget.amount
      ...
      {dialogOpen && (
        <BudgetDialog
          ...
          onOpenChange={setDialogOpen}
          budget={editBudget}
          month={editBudget?.month || new Date().getMonth() + 1}
          year={editBudget?.year || new Date().getFullYear()}
          onSaveSuccess={()=>{fetchBudgets()}}
        />
      )}
    </div>
